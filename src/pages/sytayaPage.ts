...
export function sytayaPage(): string {
  const hero = `<section class='product-hero product-hero--sytaya' aria-labelledby='sytaya-title'>${Container({ content: `<div class='product-hero__layout'><div class='product-hero__content'><p class='home-kicker'>7-дневный самостоятельный практикум</p><h1 id='sytaya-title'>СЫТАЯ</h1><p class='product-hero__lead'>Разберите свой обычный сценарий питания и соберите систему, с которой к вечеру легче оставаться сытой и спокойной рядом с едой.</p><p>Без подсчёта КБЖУ, взвешивания каждого продукта и запрета на сладкое.</p><div class='product-hero__price'><strong>1 990 ₽</strong><span>7 дней · проходите в своём темпе</span></div><div class='product-hero__actions'>${Button({ label: 'Как получить доступ', href: '#access' })}${Button({ label: 'Посмотреть программу', href: '#program', variant: 'secondary' })}</div></div>${SignatureArt('plate')}</div>` })}</section>`;
  ...
  return PageShell({ activePath: '/sytaya/', mainClassName: 'sytaya-page product-page', mainContent: [hero, SignatureDivider(), fit, program, SignatureDivider(), how, next, FAQ({ items: faq, id: 'sytaya-faq' }), access].join('') });
}
