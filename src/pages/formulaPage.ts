...
function checklist(items: string[], marker: string): string {
  return `<ul class="mt-5 space-y-3">${items
    .map(
      (item) => `<li class="flex gap-3 text-sm leading-6"><span class="font-bold text-brand" aria-hidden="true">${marker}</span><span>${escapeHtml(item)}</span></li>`,
    )
    .join('')}</ul>`;
}
      ...
      ${Container({
        content: `
          ...
          <div class="mt-9 grid gap-5 md:grid-cols-2">
            <article class="rounded-panel border border-line-strong p-6 sm:p-8">
        ...
        content: `
          ...
          <ol class="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-4">
            ${formulaProcess.steps
              .map(
                (step, index) => `
                  <li class="rounded-panel border border-line p-5 sm:p-6">
        ...
        content: `
          ...
          <div class="mt-9 grid gap-px overflow-hidden rounded-panel border border-line-strong bg-line-strong md:grid-cols-2">
            ${formulaComparison.columns
              .map(
                (column, index) => `
                  <article class="bg-canvas p-6 sm:p-8">
...
function CaseSection(): string {
  return `
    <section class="border-y border-line py-16 sm:py-20 lg:py-24" aria-labelledby="formula-case-title">
      ${Container({
        content: `
          ${SectionHeading({ id: 'formula-case-title', eyebrow: 'Кейс / отзыв', title: 'Только подтвержденная история' })}
          <div class="mt-9">${CasePreview(formulaCase)}</div>
        `,
      })}
    </section>
  `;
...
function PriceSection(): string {
  return `
    <section class="py-16 sm:py-20 lg:py-24" aria-labelledby="price-title">
      ${Container({
        content: `
          <div class="rounded-panel border border-line-strong p-6 sm:p-9 lg:p-12">
