...
export function mentorshipPage(): string {
  const hero = `<section class='product-hero' aria-labelledby='mentor-title'>${Container({ content: `<div class='product-hero__content'><p class='home-kicker'>Персональное наставничество</p><h1 id='mentor-title'>План подстраивается под вас. Не вы под план.</h1><p class='product-hero__lead'>За 3 месяца мы выстроим персональную систему питания, нагрузки и восстановления под вашу цель — и будем менять её по реакции именно вашего тела.</p><p>Я лично веду вас весь путь и каждую неделю корректирую дальнейшие действия по вашей динамике.</p><div class='product-hero__price'><strong>90 000 ₽</strong><span>за 3 месяца при полной оплате<br>или 3 × 32 000 ₽</span></div><div class='product-hero__actions'>${Button(action)}${Button({ label: 'Сначала «Формула тела»', href: '/formula-tela/', variant: 'secondary' })}</div><small>Количество мест ограничено, потому что каждого клиента я веду лично.</small></div>` })}</section>`;
  ...
  return PageShell({ activePath: '/nastavnichestvo/', mainClassName: 'mentorship-page product-page', mainContent: [hero, difference, recognition, timeline, scope, training, support, next, goal, stories, price, credit, capacity, application(), FAQ({ items: faq, id: 'mentor-faq' }), closing].join('') });
}
