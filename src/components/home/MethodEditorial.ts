        ...
        content: `
          <div class="grid gap-8 lg:grid-cols-12 lg:gap-12">
            <div class="lg:col-span-5">
              <p class="home-kicker">${escapeHtml(eyebrow)}</p>
              <h2 class="home-title" id="method-editorial-title">${escapeHtml(title)}</h2>
            </div>
            <div class="lg:col-span-6 lg:col-start-7 lg:pt-8">
              <p class="home-lead mb-0">${escapeHtml(text)}</p>
            </div>
          </div>

          <ol class="m-0 mt-12 grid list-none gap-px border border-line bg-line p-0 md:grid-cols-2 lg:mt-16 lg:grid-cols-12">
            ${pillars
              .map((pillar, index) => {
                ...
                return `
                  <li class="method-editorial__pillar flex min-w-0 flex-col justify-between p-6 sm:p-8 lg:p-10 ${surfaceClass} ${layoutClass}">
