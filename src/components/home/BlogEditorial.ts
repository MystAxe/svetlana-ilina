...
function LeadArticle(article: BlogArticle): string {
  return `
    <article class="blog-editorial__article" aria-labelledby="blog-lead-title" data-motion-item>
      ${EditorialPicture({
        image: article.image,
        className: 'min-w-0',
      ...
      <div class="mt-6 max-w-3xl">
        ...
        <h3 class="max-w-[24ch] text-balance text-subhead font-semibold text-ink" id="blog-lead-title">
          <a class="nav-link inline-flex min-h-11 items-center hover:text-brand" href="${escapeHtml(article.href)}">${escapeHtml(article.title)}</a>
        </h3>
...
function SecondaryArticle(article: BlogArticle, index: number): string {
  ...
  return `
    <article class="blog-editorial__article grid gap-5 border-t border-line-strong pt-6 sm:grid-cols-[minmax(8rem,0.42fr)_minmax(0,0.58fr)] md:grid-cols-1 lg:grid-cols-[minmax(8rem,0.42fr)_minmax(0,0.58fr)]" aria-labelledby="${titleId}" data-motion-item>
      ${EditorialPicture({
        image: article.image,
        className: 'min-w-0',
        imageClassName: 'blog-editorial__image',
        ...
        content: `
          <div class="grid gap-6 lg:grid-cols-12 lg:items-end lg:gap-10">
            <div class="lg:col-span-8" data-motion-group>
              <p class="mb-4 text-xs font-bold uppercase tracking-[0.14em] text-brand" data-motion-item>${escapeHtml(eyebrow)}</p>
              <h2 class="home-display-heading max-w-[20ch] text-ink" id="blog-editorial-title" data-motion-item>${escapeHtml(title)}</h2>
              <p class="mt-5 max-w-[60ch] text-pretty text-lead text-ink-soft" data-motion-item>${escapeHtml(text)}</p>
            </div>
            <div class="lg:col-span-4 lg:text-right" data-motion-group data-motion-offset="1"><div data-motion-item>${TextLink(action)}</div></div>
          </div>

          <div class="mt-10 grid gap-10 lg:grid-cols-12 lg:gap-12 xl:mt-14">
            <div class="lg:col-span-7" data-motion-group data-motion-offset="1">${LeadArticle(leadArticle)}</div>
            <div class="grid gap-8 md:grid-cols-2 lg:col-span-5 lg:grid-cols-1 lg:content-start" data-motion-group data-motion-offset="2">${secondaryMarkup}</div>
          </div>
        `,
      })}
    </section>
