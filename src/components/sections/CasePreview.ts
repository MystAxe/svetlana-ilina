...
export function CasePreview({ title, text, status = 'Демонстрация структуры · mock', href = '/cases/' }: CasePreviewProps): string {
  return `
    <article class="grid overflow-hidden rounded-panel border border-line-strong lg:grid-cols-12">
      <div class="image-placeholder flex min-h-64 min-w-0 items-center justify-center border-b border-dashed border-line-strong p-8 text-center lg:col-span-5 lg:border-b-0 lg:border-r" aria-label="Место для подтвержденного изображения кейса">
      ...
      <div class="min-w-0 p-6 sm:p-9 lg:col-span-7 lg:p-12">
        ...
        <div class="mt-6">${TextLink({ label: 'Посмотреть структуру кейсов', href })}</div>
