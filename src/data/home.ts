...
export const caseFeature = {
  eyebrow: 'Демонстрационный кейс',
  title: 'Как может выглядеть история изменений',
  lead: 'Рабочий пример для проверки структуры и эмоционального ритма блока.',
  prototypeLabel: 'Демонстрационные данные · не подтверждены',
  metric: '−13 кг*',
  metricLabel: 'без тренировок на первом этапе',
  story: [
    { label: 'С чем пришла', text: 'Много разрозненных попыток и отсутствие понятной последовательности.' },
    { label: 'Что изменили', text: 'Точный ход работы будет добавлен только по материалам подтвержденного кейса.' },
    { label: 'Результат', text: 'Цифра, период и контекст пока не подтверждены.' },
  ],
  image: { ...portraitPlaceholder, crop: 'portrait-close' as const, alt: 'Место для фотографии участницы кейса при наличии письменного согласия', placeholderLabel: 'Фото — только после отдельного согласия клиента' },
  action: { label: 'Посмотреть структуру кейсов', href: '/cases/' },
  publicDisclaimer: 'Цифра, история и фотография в прототипе не подтверждены и не являются обещанием результата. Реальные результаты индивидуальны.',
  meta: {
    verification: 'mock',
    publication: 'prototype-only',
    source: 'mock',
    publicLabel: 'Прототип · не является отзывом',
    editorialNotes: ['До публикации нужны первичные материалы, согласие на текст, фото и цифры.'],
