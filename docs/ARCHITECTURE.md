...
Файлы `polinaStoryPage.ts`, `svetaStoryPage.ts`, `vikaStoryPage.ts` и `oksanaStoryPage.ts` содержат только уникальные экраны и передают данные в общие секции. Это позволяет сохранять индивидуальный сценарий истории, не дублируя базовую верстку.
...
WordPress сейчас не является источником контента: публичный frontend собирается из TypeScript-данных. `scripts/sync-wordpress-theme.mjs` умеет только скопировать готовые frontend-артефакты в заранее существующую внешнюю тему. CPT, таксономия, поля редактора и PHP-шаблоны в текущий репозиторий не входят, поэтому административный слой нужно реализовать до переноса на WordPress.
