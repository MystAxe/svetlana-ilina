...
Автономный многостраничный сайт на Vite и TypeScript: главная, «Формула тела», тест, блог и раздел «Истории перемен». Production-сборка полностью статическая и публикуется без PHP, базы данных и WordPress. Локальная WordPress-тема сохранена как необязательный адаптер.
...
- Подтверждённые фото и видео подключены к историям Полины и Светы; неподтверждённые материалы Вики обозначены нейтральными заглушками без вымышленного содержимого.
- Публичное название — «Истории перемен». WordPress-интеграция остаётся необязательной; внутренняя сущность CPT `case` использует таксономию `case_problem` и rewrite `/istorii-peremen/`.
